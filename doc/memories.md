...
- **SCP財団(Wikidot)のタグ翻訳ツール**を、**静的ホスティング上のHTML + 外部JSON**で運用できるようにする。
...
- **アプリ用サーバーサイド不要**でGitHub Pagesなどに簡単にホスティングしたい要望があった。外部JSONを `fetch()` するため、ローカル確認時も `file://` 直開きではなく静的HTTPサーバー経由で開く。
...
- **index.html**
  ...
  - 辞書JSONを `fetch()` するため、GitHub Pages などの静的ホスティング、またはローカルHTTPサーバー経由で利用する。
...
- **dictionaries/ フォルダ**
  ...
  - ファイルが存在しないペアや `file://` 制限による読み込み失敗は、ログにエラーとして表示する。
